...
public class NoticeBoardInteractionRayaDorm : MonoBehaviour
{
    [Header("UI")]
    ...
    {
        if (other.CompareTag("NoticeBoard") && !_HasInteracted)
        {
            _IsInRange = true;
            ...
            if (_HasInteracted)
                ...
                _InteractText.text = "Press F To Interact With Notice Board";
        }
    }

    public void OnTriggerExit(Collider other)
    {
        if (other.CompareTag("NoticeBoard"))
        {
            _IsInRange = false;
            _InteractIndicator.SetActive(false);
        }
